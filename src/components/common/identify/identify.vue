...
export default {
  ...
  props: {
    ...
    textColor: {
      type: String,
      default: 'rgb(25,100,190)'
    },
    backgroundColor: {
      type: String,
      default: 'rgb(150,90,190)'
    },
    fontSizeMin: {
      type: Number,
      default: 32
  ...
  methods: {
    ...
    drawPic () {
      ...
      if (this.isSimple) {
        ctx.fileStyle = this.backgroundColor
      }
    ...
    drawText (ctx, txt, i) {
      ...
      if (this.isSimple) {
        y = this.contentHeight * 0.95
        deg = 0
        ctx.font = (this.fontSizeMin + this.fontSizeMax) / 2 + 'px SimHei'
      }
      // 修改坐标原点和旋转角度
      ctx.translate(x, y)
